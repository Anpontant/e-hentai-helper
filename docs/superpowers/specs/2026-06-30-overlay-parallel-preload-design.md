...
preloadPagesAhead(pages, runId):
  1. URL 確保 ensureWindowUrls(pages, runId):
     ...
     残っている先頭の未取得ページを覆うギャラリー一覧を取得
     （fetchGalleryPageUrls(galleryUrl, page, signal) を await。signal = 現在の AbortController）。
     取得後に未取得数が減らなければ（進捗なし）打ち切り（無限ループ防止）。
     ※窓が一覧ページ境界（既定20件区切り）をまたぐ場合に複数ページ取得する。
       URL が揃っていれば no-op。galleryUrl 不明なら何もしない。
  2. 並列実行: pages を map して prefetchOnePage(page, depth, runId) を一斉に起動し
     Promise.all で束ねる（depth = 配列インデックス+1、ステータス表示用）。await 間に
     バリアを置かない。呼び出し側 preloadNext は void で意図的に fire-and-forget。
```
...
- preloadedPages に page 有り（バイト取得済み）→ （showPreloadThumbs ON かつ windowImages 未登録なら
  pageImageMap[page] から Image を作り windowImages へ補完）→ setPreloadState(loaded, 'cache') 終了
...
`preloadImage(page, imageUrl, runId)` は `new Image()` のバイト先読み。`settled` フラグで
**load / error / timeout の最初の結果のみ採用**し、最初に確定したのが `onload` かつ
`runId === preloadRunId` のときだけ `preloadedPages.add(page)` / `windowImages.set(page, image)`
する。`Image` のロードは AbortController で止まらないため、タイムアウト後に遅れて発火する
`onload` が「status は failed なのに次窓では cache 扱い」という不整合や、古い run の完了が
新キャッシュへ混入するのを防ぐ。
...
- **`windowImages: Map<number, HTMLImageElement>`（窓ごと）** — 現在窓のサムネ表示用
  ...
  スキップ（`preloadedPages` 済み）ページについても、`showPreloadThumbs` が ON のときは
  `pageImageMap[page]` から（キャッシュ供給の）Image を `windowImages` に補完するため、
  重複窓でもサムネは欠落しない。
- 関数の整理:
...
- **中断**: advance/seek で `abortActivePreload()` が `preloadRunId` を進め、viewer fetch・
  画像解決・**ギャラリー一覧 fetch**（`fetchGalleryPageUrls(..., signal)` に signal を渡す）を
  abort。各 then で runId を照合して陳腐化結果を破棄。`Image` のロードは abort できないため、
  `preloadImage` は `settled` フラグ＋`onload` 時の runId 再確認でキャッシュ保存を防ぐ。
